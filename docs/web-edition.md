...
Downloads, when packaged releases become available, refer only to the full native application. The website is never presented as an installer or substitute download.
...
The native frontend remains `npm run dev` / `npm run build`. Build-time aliases ensure Tauri bridge modules are excluded from the hosted bundle.
