...
The website-only **Desktop setup** action points to `VITE_DESKTOP_PROJECT_URL` (or the repository quick-start instructions by default). No public signed installer is available yet; the link leads to the source and setup instructions.
...
Set `VITE_DESKTOP_PROJECT_URL` to the public desktop setup page when overriding the default. The native frontend remains `npm run dev` / `npm run build`; build-time aliases ensure Tauri bridge modules are excluded from the hosted bundle.
