...
export function useValidation() {
  ...
  const validate = (next: Config, ignores: string[]) => {
    try {
      ...
      if (!next.watched || next.watched.length === 0)
        return 'Add at least one folder or file to back up.';
      ...
      for (const w of next.watched) {
        ...
        if (primary.startsWith(w.path))
          return 'Backup destination cannot be inside a watched path.';
        if (w.path.startsWith(primary))
          return 'Watched path is inside backup destination; choose a different destination.';
