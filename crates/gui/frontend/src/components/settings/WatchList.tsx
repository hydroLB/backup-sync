 ...
 */
const WatchList: React.FC<Props> = ({ items, openPath, onToggleOpen, onToggleEnabled, onRemove, ignore_patterns, onAddIgnore }) => {
    ...
    return (
      <div className="watch-list">
        {items.map((w) => {
          ...
          const existingForPath = (ignore_patterns || []).filter((p) => p.startsWith(w.path.toString()));
          return (
            <div key={w.path} className="watch-card">
              <div className="watch-card__header" onClick={() => onToggleOpen(isOpen ? null : w.path)}>
              ...
              {isOpen && (
                <div className="watch-card__body">
                  <div className="muted">Status: {w.enabled ? "Backups will run on schedule" : "Not backing up until you toggle it on"}</div>
                  <div className="inline-actions">
                    <button
                      className="btn secondary"
                      onClick={(e) => { e.stopPropagation(); onToggleEnabled(w.path); }}
                    >
                  ...
                  <div>
                    <div className="muted">Exclude something inside this {w.kind === "Directory" ? "folder" : "file parent"}:</div>
                    <div className="inline-actions" style={{ marginTop: 6 }}>
                      <input
                        type="text"
                        placeholder="e.g. *.log or Cache"
                        value={excludes[w.path.toString()] || ""}
                        onChange={(e) => setExcludes({ ...excludes, [w.path.toString()]: e.target.value })}
                      />
                      <button className="btn secondary" onClick={() => addExclude(w.path.toString())}>Add exclusion</button>
                    </div>
                    ...
                    {existingForPath.length > 0 && (
                      <div className="muted" style={{ marginTop: 4 }}>Existing excludes: {existingForPath.join(", ")}</div>
                    )}
                  </div>
                </div>
              )}
