...
/// Purpose: Validates non-path configuration fields and tuning blocks.
///
/// Inputs: the config, validation limits, and a label prefix.
/// Outputs: `Ok(())` when all numeric and tuning invariants hold.
/// Ties to: config loading and backup runtime behavior.
/// Side effects: None.
/// Why: keep the top-level validation function readable by grouping numeric checks.
...
/// Purpose: Validates required path settings that are structurally invalid when empty.
///
/// Inputs: the config and label prefix.
/// Outputs: `Ok(())` when required paths are present.
/// Ties to: destination and state file layout.
/// Side effects: None.
/// Why: avoid running a cycle when basic config paths are missing.
...
/// Purpose: Validates scheduling-related fields.
///
/// Inputs: the config, limits, and label prefix.
/// Outputs: `Ok(())` when intervals are within guardrails.
/// Ties to: daemon cycle scheduling.
/// Side effects: None.
/// Why: prevent accidental busy loops or multi-month stalls.
...
/// Purpose: Validates top-level and execution tuning fields.
///
/// Inputs: the config, limits, and label prefix.
/// Outputs: `Ok(())` when execution tuning is within limits.
/// Ties to: `BackupExecutor` copy buffering, retry behavior, and state updates.
/// Side effects: None.
/// Why: cap memory usage and enforce sane retry policies.
...
/// Purpose: Validates planning tuning fields.
///
/// Inputs: the config, limits, and label prefix.
/// Outputs: `Ok(())` when planning tuning is within limits.
/// Ties to: scan planning and guardrail enforcement.
/// Side effects: None.
/// Why: prevent runaway planning or hung scans.
...
/// Purpose: Validates runtime tuning fields.
///
/// Inputs: the config, limits, and label prefix.
/// Outputs: `Ok(())` when runtime tuning is within limits.
/// Ties to: daemon loop cadence and IPC behavior.
/// Side effects: None.
/// Why: bound timeouts and keep daemon responsiveness predictable.
