...
4. Set `BACKUP_SYNC_PASSPHRASE` for privileged actions, for example `export BACKUP_SYNC_PASSPHRASE="change-me"`.
...
Auth passphrase:
- Set `BACKUP_SYNC_PASSPHRASE` in the environment used to launch the app or CLI.
- The Settings UI will prompt for the same passphrase to unlock privileged actions.
